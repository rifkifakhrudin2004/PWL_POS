    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function($level) {
                $editButton = '<a href="' . route('/level/edit', ['id' => $level->level_id]) . '" class="btn btn-primary mr-2">
                                    <i class="fa fa-pencil-alt" style="color: white; font-size: 12px;"></i>
                                </a>';

                $deleteButton = '<a href="' . route('/level/delete', ['id' => $level->level_id]) . '" class="btn btn-danger" onclick="return confirm(\'Are you sure want to delete?\')">
